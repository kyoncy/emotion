...
var appendStringReturningExpressionToArguments =
...
var createObjectSpreadLike = function createObjectSpreadLike(t, file) {
  for (
    var _len = arguments.length,
      objs = new Array(_len > 2 ? _len - 2 : 0),
      _key = 2;
    _key < _len;
    _key++
  ) {
  ...
  return t.callExpression(
    ...
    [t.objectExpression([])].concat(objs)
  )
}
...
var getStyledOptions = function getStyledOptions(t, path, state) {
  ...
  if (label) {
    var labelNode = t.objectProperty(
      t.identifier('label'),
  ...
  if (optionsArgument) {
    ...
    if (!t.isObjectExpression(optionsArgument)) {
      var prodNode = createObjectSpreadLike(
        t,
        state.file,
...
var transformCsslessObjectExpression =
  function transformCsslessObjectExpression(_ref3) {
