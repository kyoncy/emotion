...
var wrappedEnzymeSerializer = {
  ...
  print: function print(enzymeWrapper, printer) {
    ...
    if (isShallow && enzymeWrapper.root() === enzymeWrapper) {
      ...
      if (
        createSerializer.isEmotionCssPropElementType(unrendered) ||
        createSerializer.isStyledElementType(unrendered)
      ) {
        return enzymeToJsonSerializer.print(
          createSerializer.unwrapFromPotentialFragment(enzymeWrapper),
          printer
        )
      }
    }
